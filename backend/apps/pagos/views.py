...
class PagoViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def registrar(self, request):
        ...
        # 2) Calcular Σ pagos y total de orden, y cambiar estado si corresponde (SQL directo)
        with connection.cursor() as cur:
            # total calculado viene de columna persisted; leemos total y estado
            cur.execute("""
                SELECT o.total, o.id_estado
                  FROM pos.Orden o
                 WHERE o.id_orden = %s
            """, [id_orden])
            row = cur.fetchone()
